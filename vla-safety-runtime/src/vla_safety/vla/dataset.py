...
def load_demo_arrays(demo_dir: str | Path):
    ...
    聚合专家示范 (demo_*.npz) 与 DAgger 重标注数据 (dagger_*.npz)。
    """
    d = Path(demo_dir)
    files = sorted(d.glob("demo_*.npz")) + sorted(d.glob("dagger_*.npz"))
    if not files:
        raise FileNotFoundError(f"{demo_dir} 下没有 demo_*.npz; 先运行 01_gen_demos.py")
    imgs, wimgs, props, acts, ids = [], [], [], [], []
    for i, f in enumerate(files):
        ...
        wimgs.append(z["wrist_images"])
        props.append(z["proprios"])
        acts.append(z["actions"])
        ids.append(np.full(n, i, dtype=np.int64))
    return (np.concatenate(imgs), np.concatenate(wimgs), np.concatenate(props),
            np.concatenate(acts), np.concatenate(ids))


class DemoDataset(Dataset):
    def __init__(self, images, wrist_images, proprios, actions, train: bool,
    ...
    def _norm(self, img_u8: np.ndarray, gain: float) -> torch.Tensor:
    ...
    def __getitem__(self, i: int):
        gain = (1.0 + self.rng.uniform(-self.jitter, self.jitter)
                if (self.train and self.jitter > 0) else 1.0)
        return (self._norm(self.images[i], gain),
                self._norm(self.wrist_images[i], gain),
...
def make_splits(demo_dir: str | Path, val_frac: float, seed: int):
    ...
    images, wrist, proprios, actions, demo_ids = load_demo_arrays(demo_dir)
    ...
    train_ds = DemoDataset(images[~val_mask], wrist[~val_mask],
                           proprios[~val_mask], actions[~val_mask],
                           train=True, seed=seed)
    val_ds = DemoDataset(images[val_mask], wrist[val_mask],
                         proprios[val_mask], actions[val_mask],
